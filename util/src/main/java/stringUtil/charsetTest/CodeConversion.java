...
public class CodeConversion {
  ...
  private static final String CodeToWord(String text) {
    ...
    if (Utf8codeCheck(text)) {
      ...
      try {
        result = new String(code, "UTF-8");//---------将字符数组转换成汉字---------
      }catch (UnsupportedEncodingException ex) {
        result = null;
      }
    }
  ...
  public static void main(String[] args) {
    ...
    if(CodeConversion.isUtf8Url(url)){
      System.out.println(CodeConversion.Utf8URLdecode(url));
    }else{
...
public class 编码问题 {
	*//**
	 ...
	 * @return
	 *//*
	public static final String ISO02GB(String text){
		String result=null;
		try {
			result = new String(text.getBytes("ISO-8859-1"),"GB2312");
